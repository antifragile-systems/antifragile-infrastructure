resource "aws_subnet" "antifragile-infrastructure" {
  ...
  cidr_block                      = "${cidrsubnet(var.aws_cidr_block, 8, var.is_ascending_order ? count.index : 255 - count.index)}"
  assign_ipv6_address_on_creation = true
  ipv6_cidr_block                 = "${cidrsubnet(var.aws_ipv6_cidr_block, 8, var.is_ascending_order ? count.index : 255 - count.index)}"
